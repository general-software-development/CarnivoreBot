...
@typecheck_complex
def deepSize(value: Any, seen: set[int] | None = None) -> int:  # pylint: disable=too-many-return-statements
    if seen is None:
        seen = set()
    ...
    if isinstance(value, (type, SimpleNamespace)):
    ...
    while True:
        ...
        for (name, subsystem) in data.items():
            if deepSize(subsystem) >= subsystem_size_limits.get(name, 512) * 15:
                logger.critical(f"Size of runtime data subsystem '{name}' exceeds the limit of"
                                + f"{toHumanReadable(subsystem_size_limits.get(name, 512))} by 15x (or more),"
                                + f"currently occupying {toHumanReadable(deepSize(subsystem))}")

            elif deepSize(subsystem) >= subsystem_size_limits.get(name, 512):
                logger.warning(f"Size of runtime data subsystem '{name}' exceeds the limit of {toHumanReadable(subsystem_size_limits.get(name, 512))},"
                               + f" currently occupying {toHumanReadable(deepSize(subsystem))}")
