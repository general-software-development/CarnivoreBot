...
async def load_model():
    ...
    class Model(nn.Module):
        def __init__(self, vocab_dim, d_model=34, num_classes=2, num_cls_tokens=4): # pylint: disable=unused-argument
            ...
    ...
    if get_device() == "cpu":
        logger.warning("Running model on the CPU. This may have degraded performance. Applying int8 quantization..")
        MODEL = quantize_dynamic(MODEL, {torch.nn.Linear}, dtype=torch.qint8)
    else:
        logger.info("Running model on the GPU in float16.")
...
class CheckIsSpamCommand(CommandABC):
    ...
    async def _onRunCommand(self, message: discord.Message, _: Iterable[str]):
